...
class DataTrainingArguments:
    ...
    train_file: Optional[str] = field(
        default=None, metadata={"help": "Path to training jsonl/csv manifest file."}
    )
    validation_file: Optional[str] = field(
        default=None, metadata={"help": "Path to validation jsonl/csv manifest file."}
    )
...
def main():
    ...
    if data_args.train_file is not None and os.path.exists(data_args.train_file):
        logger.info(f"Loading train dataset from local manifest: {data_args.train_file}")
    ...
    if data_args.validation_file is not None and os.path.exists(data_args.validation_file):
        logger.info(f"Loading eval dataset from local manifest: {data_args.validation_file}")
        raw_datasets["eval"] = load_dataset("json", data_files=data_args.validation_file, split="train")
    ...
    # Ensure audio column has absolute paths and is cast to Audio(sampling_rate=16000)
    for split_k in list(raw_datasets.keys()):
        if data_args.audio_column_name in raw_datasets[split_k].column_names:
            first_val = raw_datasets[split_k][0][data_args.audio_column_name]
            if isinstance(first_val, str):
                def make_audio_absolute(batch):
                    batch[data_args.audio_column_name] = [
                        os.path.abspath(p) if isinstance(p, str) and not os.path.isabs(p) else p
                        for p in batch[data_args.audio_column_name]
                    ]
                    return batch
                raw_datasets[split_k] = raw_datasets[split_k].map(make_audio_absolute, batched=True)
                raw_datasets[split_k] = raw_datasets[split_k].cast_column(
                    data_args.audio_column_name, Audio(sampling_rate=16000)
                )
    ...
    with training_args.main_process_first(desc="apply_weight_norm"):
        ...
        if model_args.override_vocabulary_embeddings:
            ...
            if old_num_tokens != new_num_tokens:
                logger.info(f"Resizing token embeddings from {old_num_tokens} to {new_num_tokens} with normal initialization...")
                new_embeddings = torch.nn.Embedding(new_num_tokens, embedding_dim, padding_idx=config.pad_token_id)
                init_std = getattr(config, "initializer_range", 0.02)
                torch.nn.init.normal_(new_embeddings.weight, mean=0.0, std=init_std)
                with torch.no_grad():
